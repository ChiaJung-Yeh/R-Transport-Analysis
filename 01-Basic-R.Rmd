...
<!--#description This chapter systematically introduces the basic syntax of the R language, including data types, objects, text processing methods, control flow, and methods for data import and export. -->

# (PART\*) Basic Data Analysis {-}

# **Basic Syntax in R**
This chapter systematically introduces the essential basic syntax of the R language, including data types (objects), text processing methods, control flow, and methods for reading and exporting data. Readers are encouraged to become familiar with the functions associated with these objects to facilitate subsequent code development.
...
Most of the functions mentioned above enable the argument `na.rm=` to be specified, which determines whether `NA` values should be ignored. If `NA` values are present in the data and this argument is not specified, the returned result will also be `NA`. An example is shown below:
...
vector_calculation=data.frame(ope_name=c("Absolute value", "Square root", "Round", "Truncate", "Floor", "Logarithm", "Exponential", "Standardization", "cumulative sum"), operation=c("`abs(vec5)`","`sqrt(vec4)`","`round(vec5, digits=1)`","`ceiling(vec5)`","`floor(vec5)`","`log(vec4)`","`exp(vec4)`","`scale(vec4)`","`cumsum(vec4)`"),
...
* 功能類似 Excel 的工作表  
* 資料分析中最常見的資料型態  
* 可利用`dplyr`套件處理資料（參見[資料清洗與處理](#資料清洗與處理)章節）

<p style="color:#003D79;font-size:18px;line-height:2">**⌾ Contruction**</p>  
資料框的建立如下程式碼：
...
<p style="color:#003D79;font-size:18px;line-height:2">**⌾ 設定資料欄位名稱**</p>  
設定資料欄位名稱亦可透過`colnames()`達成之，程式碼撰寫如下。
...
### 日期與時間
在資料處理中，設定日期與時間的格式有助於後續計算時間差、繪製時間序列圖等，而在 R 語言中，可以利用`base`、`lubridate`套件處理時間資料。請注意，由於兩套件的部分函式名稱相同，故建議使用函式時寫清楚套件的名稱，如`lubridate::XXXX()`。

<p style="color:#003D79;font-size:18px;line-height:2">**⌾  基礎設定**</p>  
...
```{r setlocale2, echo=T, eval=F}
Sys.setlocale(category="LC_ALL", locale="en")
```

此外，時區亦為重要的資訊，在日期與時間的函式中皆可設定之，數個常見的時區設定如表\@ref(tab:timezone)所示。

```{r timezone, echo=F, eval=T}
temp=data.frame(timezone=c("標準時間","臺北時區","東京時區","美國東部標準時間","坎培拉時間"), tz_code=c("UTC","Asia/Taipei","Asia/Tokyo","US/Eastern","Australia/Canberra"), code=c("UTC","CST","JST","EDT","AEDT"))

kbl(temp, booktabs=T, escape=F, caption="時區格式範例", col.names=c("時區","全名","縮寫"))%>%
  kable_styling(bootstrap_options=c("striped", "hover"), font_size=14)%>%
  row_spec(0, bold=T, color="white", background="#8E8E8E")
```

其他時區的設定請參照[此](https://www.wikiwand.com/en/List_of_tz_database_time_zones)。

而時間與日期的格式亦有通用的格式設定方式，常見者彙整如表\@ref(tab:timeformat)所示。
```{r timeformat, echo=F, eval=T}
temp=data.frame(code=c("%Y","%y","%B","%b","%m","%d","%H","%h","%M","%S","%p","%Z"),
                name=c("年（四位數）","年（二位數）","月（全稱）","月（縮寫）","月（數字）","日","時（24 小時制）","時（12 小時制）","分","秒","AM/PM","時區"))

temp=cbind(temp[1:6,], temp[7:12,])
kbl(temp, booktabs=T, escape=F, caption="時間格式範例", col.names=c("代號","意義","代號","意義"), align="cccc")%>%
  kable_styling(bootstrap_options=c("striped", "hover"), font_size=14)%>%
...
<p style="color:#003D79;font-size:18px;line-height:2">**⌾  建立日期與時間**</p>  
建立日期可利用`as.Date()`函式，其中只要輸入日期格式即可，另可加入`format=`引數，用以確保正確的格式，程式碼撰寫如下。
...
建立時間可利用`as.POSIXct()`函式，其中只要輸入完整的日期與時間即可（日期若忽略，系統會自動填補為當下的電腦日期），另可加入`format=`與`tz=`引數，用以確保正確的時間格式。程式碼撰寫如下。
...
若時間格式並非上述典型的形式，則必須自行設定`format=`引數，並參照表\@ref(tab:timeformat)填入適當的格式，範例如以下程式碼。**_另請注意，若文字中含有「英文」，請務必確保電腦的文字及地區設定為`LC_ALL`，否則會出錯！！_**
...
<p style="color:#003D79;font-size:18px;line-height:2">**⌾  回傳日期時間資訊**</p>
回傳日期與時間相關資訊的函式彙整如表\@ref(tab:time-retrieve)所示，並以前文的`time_str1`資料為例。

```{r time-retrieve, echo=F, eval=T}
temp=data.frame(fun=c("","`year()`","`month()`","`day()`","`week()`","`wday()`","`hour()`","`minute()`","`second()`"),
                meaning=c("","年","月","日","週","星期","時","分","秒"),
...
kbl(temp, booktabs=T, escape=F, caption="回傳日期時間範例", col.names=c("函式","意義","範例（以<span style='color:black'>`time_str1`</span>為例）"))%>%
  kable_styling(bootstrap_options=c("striped", "hover"), font_size=14)%>%
  row_spec(0, bold=T, color="white", background="#8E8E8E")%>%
  row_spec(1, bold=T, color="black", background="#ECFFFF", monospace=T)
```

其中`wday()`星期函式所回傳的結果並非實際的星期，乃因該函式預設以「星期日」為一週的首日。此外，`month()`與`wday()`兩函式皆可設定回傳中英文、全名與縮寫等詳細資訊，完整參數設定詳如表\@ref(tab:time-retrieve)。其中`label=`引數設定是否回傳文字，否則預設為數值；`abbr=`引數設定是否回傳縮寫，此引數僅在`label=T`有效；`locale=`引數設定語言，如`en`為英文，而`zh_TW.UTF-8`為臺灣中文。
...
temp=data.frame(fun=c('`month(日期向量)`','`month(日期向量, label=T, locale="en")`','`month(日期向量, label=T, abbr=F, locale="en")`','`month(日期向量, label=T, locale="zh_TW.UTF-8")`','`wday(日期向量)`','`wday(日期向量, week_start=1)`','`wday(日期向量, label=T, locale="en")`','`wday(日期向量, label=T, abbr=F, locale="en")`','`wday(日期向量, label=T, locale="zh_TW.UTF-8")`','`wday(日期向量, label=T, abbr=F, locale="zh_TW.UTF-8")`'),
...
kbl(temp, booktabs=T, escape=F, caption="`month()`與`wday()`函式引數設定", col.names=c("函式與引數","回傳結果"))%>%
  kable_styling(bootstrap_options=c("striped", "hover"), font_size=14)%>%
  row_spec(0, bold=T, color="white", background="#8E8E8E")%>%
  column_spec(1, width="11cm")%>%
  pack_rows("`month()` 月", 1, 4, bold=T, background="#ECFFFF")%>%
  pack_rows("`wday()` 星期", 5, 10, bold=T, background="#ECFFFF")
```
