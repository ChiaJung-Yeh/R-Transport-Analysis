...
<H2 id="read-csv"><p style="font-size:18px;line-height:1">**<u>Read from the local directory</u>**</p></H2>
...
The syntax of reading Excel file is as follows. The `PATH` argument specifies the path of the Excel file (see how to write the <A href="#read-csv">file path</A>), while the `sheet=` argument can be either the name of the worksheet or its index. It is not necessary to specify the `sheet=` argument if the workbook contains one sheet only.

```{r readxl-read-excel-code, echo=T, eval=F}
read_excel("PATH.xlsx", sheet="SHEET_NAME/SHEET_INDEX")
```

To begin with an example, please download the Excel file [here](https://raw.githubusercontent.com/ChiaJung-Yeh/R-Transport-Analysis/master/Data/nsw_busperformance_reports.xlsx). The dataset contains bus performance data from New South Wales, Australia. Reading the data can be straightforward by specifying the name of workbook and worksheet, as shown below.

```{r readxl-example, echo=T, eval=F}
# specify the worksheet name
bus_performance=read_excel("nsw_busperformance_reports.xlsx", sheet="BusData")

# specify the worksheet index
bus_performance=read_excel("nsw_busperformance_reports.xlsx", sheet=1)
```


The `bus_performance` data is shown below.

```{r read-excel-show, echo=F, eval=T, message=F, warning=F}
bus_performance=read_excel("Data/nsw_busperformance_reports.xlsx", sheet="BusData")
datatable(bus_performance, options=list(pageLength=5, scrollX=T, headerCallback = DT::JS(
    "function(thead) {",
    "  $(thead).css('font-size', '0.7em');",
    "}"
  )))%>%
  formatStyle(columns=c(1:ncol(bus_performance)), fontSize="12px")%>%
  formatRound(c("OTR","Percentage of services cancelled","Untracked trips"), digits=3)
```
