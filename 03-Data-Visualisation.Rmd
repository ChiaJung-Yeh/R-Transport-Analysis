...
windowsFonts(A=windowsFont("Tw Cen MT"))
...
# **Data Visualisation**
Data visualisation plays a crucial role in both ex- and post- data analysis. Prior to data analysis, it helps organise complex and unstructured data into statistically meaningful graphical representations, enabling researchers to explore patterns, identify trends, and detect potential relationships within the data. After the analysis, visualisation serves as an effective medium for communicating results, helping to convey the key findings and maintain a coherent narrative throughout the analytical process. This chapter introduces data visualisation techniques in the R programming language, with particular emphasis on the use of the `ggplot2` package. Various types of charts and their corresponding coding approaches are presented, along with detailed explanations of graphical customisation options that allow figures to be refined and tailored for clearer and more informative presentation.

The `ggplot2` package is one of the most widely used visualisation tools in R. The term “gg” refers to the **G**rammar of **G**raphics, indicating that plots generated with this package are constructed according to a systematic set of rules, analogous to the grammatical structure of a language. The `ggplot2` package provides a wide range of functions for producing different types of charts, while also offering extensive flexibility to adjust graphical elements and styles, thereby improving the clarity and interpretability of the data.

In `ggplot2`, the plotting framework is primarily composed of four fundamental components: data and aesthetic mappings (axes and variables), geometric objects (chart types), scale settings, and theme settings. The overall structure of the plotting framework is illustrated in Figure \@ref(fig:ggplot2-fig).

```{r ggplot2-fig, echo=F, eval=T, out.width="50%", fig.align="center", fig.cap="Structure of plot using `ggplot2`"}
include_graphics("./figure/ggplot2.png")
```

Taking Figure \@ref(fig:fig-exp) as an example, the graphical elements controlled by each component are summarised as follows:

* <span style="color:#BF9000">**Data and axes:**</span> The input dataset is the iris dataset, with sepal length (`Sepal.Length`) specified as the horizontal axis and sepal width (`Sepal.Width`) specified as the vertical axis.  
* <span style="color:#002060;">**Geometric object:**</span> A scatter plot is used.  
* <span style="color:#548235;">**Scale settings:**</span> Petal width (`Petal.Width`) is mapped to a colour gradient, and petal length (`Petal.Length`) is mapped to the size of the points.  
* <span style="color:#C55A11;">**Theme settings:**</span> The colour and size of the axis text are customised, and the legend text size, weight, and background colour are specified.

```{r fig-exp, echo=F, eval=T, out.width="70%", fig.align="center", fig.cap="Example of plotting using `ggplot2` package"}
...
This chapter introduces commonly used chart types and systematically summarises the core plotting concepts of the `ggplot2` package. However, it is not possible to cover all chart types or the full range of graphical customisation options. Readers are therefore encouraged to search for relevant resources online using appropriate keywords. By examining and adapting example code from others, one can effectively learn additional plotting techniques and improve their visualisation skills.

In addition, the following resources provide rich galleries of R visualisations, many of which include complete code examples that can serve as useful references for learning and implementation:
...
This section summarises commonly used chart types (as shown in Table \@ref(tab:tab-export-plot)) and introduces the code executed to create each type of chart. Through different forms of data visualisation, various characteristics and patterns within the data can be effectively presented.
...
kable(ggplot_summary, booktabs=T, escape=F, caption="Types of chart", col.names=c("Chart Type","Features/Applications","Packages","Functions","Illustration"), align=c("llc"))%>%
  ...
  footnote("The **bold text** in the Package column indicates packages that can be integrated with the `ggplot2` package.")
```

### **Initialisation** {-}
As mentioned earlier, `ggplot2` follows the grammar of graphics. Its most fundamental elements are **data and axes**, which serve as the "canvas" before drawing various types of charts. This canvas can be generated using the `ggplot()` function, whose basic structure is shown below.

```{r ggplot-code, echo=T, eval=F}
ggplot(DATA, aes(x=, y=))
```

Using the `iris` dataset as an example, we create a canvas with **Sepal.Length** as the x-axis and **Sepal.Width** as the y-axis. The code for setting up this basic canvas is as follows.
...
<H2 id="ggplot-aes"><p style="color:#003D79;font-size:18px;line-height:2">**⌾ `aes()`: Set the aesthetics**</p></H2>

`aes` stands for "aesthetics", and it is used to define elements in a plot that **vary according to the data**. For example, when drawing points or lines in a chart, their positions must depend on the data coordinates, so the arguments `x=` and `y=` must be placed inside the `aes()` function. 

Similarly, if different data points or lines should be assigned to different colours (`color=`), sizes (`size=`), line widths (`linewidth=`), or line types (`linetype=`), these properties must be controlled by the data and therefore should also be specified within this function.

In summary, **arguments that change with the data must be placed inside `aes()`, while arguments that remain constant across the plot should be placed outside `aes()`.**

<H2 id="ggplot-data"><p style="color:#003D79;font-size:18px;line-height:2">**⌾ Data and axes**</p></H2>
Please note that specifying the data and axes does not necessarily have to be done within the `ggplot()` function; it can also be placed inside the chart-type function `geom_XX()`, and the result will be exactly the same. The syntax below yield identical outputs.  

```{r geom-setting-data, echo=T, eval=F}
# axes set in ggplot() function
ggplot(DATA, aes(x=, y=))+
  geom_XX()

# axes set in geom_XX() function
ggplot()+
  geom_XX(data=DATA, aes(x=, y=))
```

If different chart types applies the same data and axes when plotting multiple layers, axes can be specified directly within the `ggplot()` function. However, if these layers use different data or axes, it is recommended to specify them separately within each `geom_XX()` function when specifying the chart type.


### **Symbol Settings** {-}
In R, different types of charts may require settings such as shape (`shape=`), line type (`linetype=`), and colour (`color=`). The codes for these settings are briefly introduced below.

Figure \@ref(fig:geom-setting-shape) lists 26 default codes for point shapes in R, and it can be specified directly by assigning the corresponding numeric code. In addition, custom symbols can also be used For example, setting `"X"` displays the symbol “X” as the point shape.

<H2 id="geom-setting-shape"></H2>
```{r geom-setting-shape, echo=F, eval=T, out.width="65%", fig.align="center", fig.cap="Code of point shapes in R"}
temp=data.frame(id=c(0:25))%>%
  mutate(x=(id)%%6+1, y=5-(id)%/%6+1)
ggplot()+
  geom_point(data=temp, aes(x=x, y=y, shape=as.character(id)), stroke=1, size=7, fill="#FFFF37", show.legend=F)+
  scale_shape_manual(limits=factor(temp$id), values=temp$id)+
  geom_text(data=temp, aes(x=x, y=y+0.3, label=id), family="A", size=5.5)+
  theme_void()
```

There are 7 default codes for line types in R, as listed in Figure \@ref(fig:geom-setting-linetype).

<H2 id="geom-setting-linetype"></H2>
```{r geom-setting-linetype, echo=F, eval=T, out.width="70%", fig.align="center", fig.cap="Code of line shapes in R"}
...
ggplot()+
  ...
  geom_text(data=temp, aes(x=1.5, y=id+0.3, label=paste0(id, " - ", linetype)), family="A", size=5.5)+
  theme_void()
```

Colours can be specified either by using predefined colour names, as listed in Figure \@ref(fig:geom-setting-color), or by directly providing hexadecimal colour codes. For reference, see the [colour code table](https://www.ifreesite.com/color/).

<H2 id="geom-setting-color"></H2>
```{r geom-setting-color, echo=F, eval=T, out.width="100%", fig.align="center", fig.cap="Predefined colour name"}
...
ggplot(temp, aes(x=x, y=y, fill=color)) +
  ...
  geom_text(aes(label=color, color=fontcolor), family="A", size=3, show.legend=F)+
  scale_color_manual(values=c("black"="black", "white"="white"))+
  theme_void()
```

With the above basic understanding of chart settings, we will now use a variety of datasets and examples to demonstrate the functions of different chart types and their corresponding code implementations.



### Scatter Plot
A scatter plot is a graphical method for illustrating the relationship between two continuous variables. The horizontal (x) and vertical (y) axes represent different continuous variables, while individual points depict their paired values within a two-dimensional space. This visualisation can be created using the `geom_point()` function, with the general function structure shown below.

```{r geom-point-code, echo=T, eval=F}
geom_point(color=, shape=, size=, stroke=, fill=, alpha=)
```

The settings of the above arguments are summarised in Table \@ref(tab:geom-point-code-tab).

```{r geom-point-code-tab, echo=F, eval=T}
gp_code=data.frame(argument=c("`color=`","`shape=`","`size=`","`stroke=`","`fill=`","`alpha=`"),
                   setting=c("Set the Colour of the points", "Set the shape of the points", "Set the size of the points", "Set the thickness of the point outline", "Set the fill Colour of the points", "Set the transparency of the points"), explain=c("Refer to <A href='#geom-setting-color'>colour setttings</A>","Refer to <A href='#geom-setting-shape'>shape settings</A>","Values","Values","Refer to <A href='#geom-setting-color'>colour settings</A>. Note that only the hollow shapes（e.g., 21~25 in <A href='#geom-setting-shape'>Shape Settings</A> are effective.","Numeric value between 0 (fully transparent) and 1 (fully opaque)"))

kable(gp_code, booktabs=T, escape=F, caption="Argument settings in `geom_point()`", col.names=c("Arguments","Usage","Note"))%>%
  kable_styling(bootstrap_options=c("striped", "hover"), font_size=14)%>%
  row_spec(0, bold=T, color="white", background="#8E8E8E")
```


<p style="color:#003D79;font-size:18px;line-height:2">**⌾ Basic scatter plot**</p>
Using the `iris` dataset as an example, plot a scatter plot with `Sepal.Length` on the x-axis and `Sepal.Width` on the y-axis. The code is written as follows.
...
<p style="color:#003D79;font-size:18px;line-height:2">**⌾ Revise the styles**</p>
To customise the appearance of the points, you can adjust them using the arguments listed in Table \@ref(tab:geom-point-code-tab).
...
<p style="color:#003D79;font-size:18px;line-height:2">**⌾ Gradient colour scatter plot**</p>
A scatter plot typically represents two-dimensional data and is most often used for continuous variables. However, by assigning attributes such as colour or size to the points, additional dimensions of data can be visualised. 

In a gradient-style scatter plot, any aesthetic that **varies with the data** must be specified **inside** the <A href="#ggplot-aes">`aes()`</A> function. Conversely, if an aesthetic does **not vary with the data**, it should be placed **outside** the <A href="#ggplot-aes">`aes()`</A> function.

Building on the previous example, colour can be used to distinguish species (`Species`), while point size can represent petal width (`Petal.Width`). The point shape is kept fixed as circles, and therefore its corresponding argument should be specified **outside** the <A href="#ggplot-aes">`aes()`</A> function. The code is shown below.
...
In the figure, “Species” is a categorical variable, while “Petal.Width” is a continuous variable. By incorporating gradient aesthetics in the scatter plot, additional dimensions of the data can be represented, enriching the overall information conveyed.

From the plot, it can be observed that the setosa category (<span style="color:#F8766D">red dots</span>) has relatively smaller petal widths (indicated by point size) and shorter sepal lengths (x-axis) compared with the other two species, making it clearly distinguishable. This demonstrates how gradient-enhanced scatter plots can help identify important variables for data classification, which is a key step in data science.

To adjust the colour gradient, refer to [this](#gradient-colour); to modify the size scale of the points, refer to [this](#size-scale).


<p style="color:#003D79;font-size:18px;line-height:2">**⌾ Gradient colour scatter plot: adjust the legend order**</p>
For adjusting the order of the legend, please first refer to [this](#adjust-legend). By default, legend entries are arranged in alphabetical order. To customise the legend order into a specified sequence, the [`factor()`](#factor) function can be used to define the levels of the categorical variable. 

For example, to set the legend order as “virginica, versicolor, setosa”, the code is written as follows.
...
<p style="color:#003D79;font-size:18px;line-height:2">**⌾ Add text and label**</p>
To add text annotations to a plot, the `geom_text()` function should be used (see [this](#add-texts-and-labels)). 

For example, to label the centroid of each species (`Species`), the following code first calculates the centre point of each group using `group_by() %>% summarise()`, and then adds the corresponding text labels to the plot.

```{r geom-point-eg5, echo=T, eval=T, out.width="70%", fig.align="center"}
# derive the centre point of each species
...
Alternatively, using `geom_label()` function to add the labels. The corresponding code is shown below.
...
<p style="color:#003D79;font-size:18px;line-height:2">**⌾ Add trend line**</p>
A scatter plot is commonly used to illustrate the relationship between two continuous variables, allowing underlying trends to be inferred. When creating such plots, it is often helpful to include a trend line to guide interpretation of the relationship between variables. 

A trend line can be added using the `geom_smooth()` function, with the general structure of the function shown as follows.

```{r geom-smooth-code, echo=T, eval=F}
geom_smooth(method, se=, span=, level=, color=, linetype=, linewidth=, fill=, alpha=)
```

The settings of all arguments is shown in Table \@ref(tab:geom-smooth-code-tab)。
...
gs_code=data.frame(argument=c("`method=`","`formula=`","`se=`","`level=`","`span=`","`color=`","`linetype=`","`linewidth=`","`fill=`","`alpha=`"),
                   setting=c("Set the regression model for the trend line", "Set the regression formula", "Set whether to display the standard error of the trend line", "Set the confidence level", "Set the smoothing span of the regression", "Set the colour of the trend line", "Set the line type of the trend line", "Set the thickness of the trend line", "Set the colour of the standard error band", "Set the transparency of the standard error band"), explain = c("Linear regression (`lm`) or generalised linear models (`glm`) can be used", "Default is `y ~ x`", "Default is `TRUE`; set to `FALSE` to hide the standard error", "Default is 95% (`0.95`) confidence level", "Controls the smoothness of the curve; larger values produce smoother lines, smaller values follow the data more closely", "Refer to <A href='#geom-setting-color'>colour settings</A>", "Refer to <A href='#geom-setting-linetype'>line type settings</A>", "Numeric value", "Refer to <A href='#geom-setting-color'>colour settings</A>", "Numeric value between 0 (fully transparent) and 1 (fully opaque)"))

kable(gs_code, booktabs=T, escape=F, caption="Argument settings of `geom_smooth()`", col.names=c("Arguments","Usage","Note"))%>%
  kable_styling(bootstrap_options=c("striped", "hover"), font_size=14)%>%
  column_spec(2, width="6.5cm")%>%
  row_spec(0, bold=T, color="white", background="#8E8E8E")
```

As an example of adding a linear trend line, the `method =` argument should be set to `lm`, which specifies a linear regression model. The corresponding formula is $y = \beta x$ (`y ~ x`). The code is written as follows. 

Note that when using linear regression, there is no need to specify the `span =` argument, as it is only applicable to smoothing methods for curved lines.
...
When plotting a curved trend line, different values of the `span =` argument will produce different smoothing results, as shown in Figure \@ref(fig:geom-smooth-eg2). It can be observed that larger values lead to a smoother trend line.

```{r geom-smooth-eg2, echo=F, eval=T, out.width="85%", fig.align="center", fig.cap="Argument `span=` in `geom_smooth()` function", warning=F, message=F}
plot_list=list()
...
<p style="color:#003D79;font-size:18px;line-height:2">**⌾ Add jitter points**</p>
In some cases, data points may overlap, making them difficult to distinguish visually and harder to assess the true concentration of observations. When multiple points overlap into a single point, densely clustered data may be mistakenly perceived as sparse. This situation commonly occurs with integer or categorical data.

For example, using the `mpg` dataset, a scatter plot can be created with the number of cylinders (cyl) on the x-axis and highway miles per gallon (hwy) on the y-axis, as shown below.
...
Since both the number of cylinders and highway miles per gallon in this dataset are positive integers, many data points overlap, making them difficult to observe clearly. In such cases, `geom_jitter()` can be used to introduce random variation, allowing points to be slightly shifted to nearby positions. This makes the distribution of the data easier to visualise without altering the overall pattern. The general structure of the `geom_jitter()` function is as follows.

```{r geom-jitter-code, echo=T, eval=F}
geom_jitter(width=, height=)
```

The `width =` argument controls the amount of horizontal variation. For example, `width = 0.1` allows points to shift randomly up to 10% to the left or right along the x-axis. The `height =` argument controls the amount of vertical variation.

In addition, all arguments available in `geom_point()` can also be applied to `geom_jitter()`. For instance, to adjust colour or size, we may use the arguments listed in Table \@ref(tab:geom-point-code-tab).

```{r geom-jitter-eg, echo=T, eval=T, out.width="70%", fig.align="center"}
# use duplicated() function to chek number of duplicated data
sum(duplicated(mpg[, c("cyl","hwy")]))

# dd random noise in jitter plot
ggplot(mpg, aes(cyl, hwy))+
  geom_jitter(width=0.2, height=0.2)
```

From the jitter plot shown above, it is evident that using `geom_jitter()` makes the data easier to distinguish. However, it is important to note that the `width =` and `height =` arguments have a significant impact on the visualisation. Larger values increase the apparent continuity of the data, but may also cause points to deviate too far from their true positions. Therefore, for different datasets, these two arguments should be adjusted through multiple trials to achieve an optimal balance between clarity and faithful representation of the original distribution. 

In addition, since the jittering process is random, the output may vary each time. To ensure reproducibility, a random seed can be set using [`set.seed()`](https://r-coder.com/set-seed-r/) before plotting, so that the same random variation is applied across runs.


### Line Chart
A line chart is used to illustrate the trend between two continuous variables, or to show how a continuous variable changes over time. This can be achieved using the `geom_line()` function, with the general structure of the function shown below.

```{r geom-line-code, echo=T, eval=F}
geom_line(color=, linetype=, linewidth=, alpha=, group=)
```

The settings of all arguments in `geom_line()` function are shown in Table \@ref(tab:geom-line-code-tab)。

```{r geom-line-code-tab, echo=F, eval=T}
gl_code=data.frame(argument=c("`color=`","`linetype=`","`linewidth=`","`alpha=`","`group=`"),
                   setting=c("Set the colour of the line", "Set the line type", "Set the thickness of the line", "Set the transparency of the line", "Set the grouping of the line"), explain = c("Refer to <A href='#geom-setting-color'>colour settings</A>", "Refer to <A href='#geom-setting-linetype'>line type settings</A>", "Numeric value", "Numeric value between 0 (fully transparent) and 1 (fully opaque)", "Specify the grouping variable (if set to 1, all data will be connected into a single line)"))

kable(gl_code, booktabs=T, escape=F, caption="Argument settings in `geom_line()`", col.names=c("Arguments","Usage","Note"))%>%
  kable_styling(bootstrap_options=c("striped", "hover"), font_size=14)%>%
  row_spec(0, bold=T, color="white", background="#8E8E8E")
```

<p style="color:#003D79;font-size:18px;line-height:2">**⌾ Basic line chart**</p>

Using the `airquality` dataset as an example, a line chart can be created with date (Month & Day) on the x-axis and temperature (Temp) on the y-axis. The code is written as follows. 

The [`as.Date()`](#date-and-time) function is used to construct date values, and the [`str_pad()`](#text-processing) function ensures consistent string lengths. In addition, the `geom_line()` function must include the argument `group = 1` to indicate that all observations should be connected into a single line.

```{r geom-line-eg1, echo=T, eval=T, out.width="70%", fig.align="center"}
# pre-processing the airquality data (create the date)
...
In the figure, the x-axis represents date values. To modify the display format of the time axis, please refer to [axis scale](#axis-scale).

<p style="color:#003D79;font-size:18px;line-height:2">**⌾ Multi-line Chart**</p>
The previous example displays only a single line. However, in many cases, it is useful to present multiple lines within the same chart to compare data across different groups. To achieve this, the `group =` argument in the `geom_line()` function must be set to a specified grouping variable.

Using the `airquality` dataset again as an example, suppose we want to draw separate lines for each month and assign different colours accordingly. In this case, the `group =` argument should be set to `Month` and placed inside the <A href="#ggplot-aes">`aes()`</A> function, indicating that the grouping varies with the data. 

Since each line is assigned a different colour, the `colour =` argument must also be specified within the <A href="#ggplot-aes">`aes()`</A> function. Finally, if the line width is fixed, the `linewidth =` argument should be placed outside the <A href="#ggplot-aes">`aes()`</A> function. Based on this logic, the code is written as follows.
...
Note that in the above code, since the plot is grouped by month, `Month` should be converted into a character or factor variable. To adjust the colour gradient, please refer to [this](#colour-sacle).

<p style="color:#003D79;font-size:18px;line-height:2">**⌾ Combining line chart and scatter plot**</p>
When presenting data trends, in addition to using a line chart, it is often helpful to overlay a scatter plot to make the individual data points more explicit. Using the `airquality` dataset as an example, the code is written as follows.

In this code, different point shapes are assigned to different months; therefore, the `shape =` argument in `geom_point()` must be placed in the <A href="#ggplot-aes">`aes()`</A> function, as it varies with the data. In contrast, the point size remains constant across months, so the `size =` argument should be specified outside the <A href="#ggplot-aes">`aes()`</A> function.
...
### Bar Chart
A bar chart is commonly used to display summary statistics across categories, such as total revenue by month or students’ scores, enabling clear comparisons between groups. It can also be applied to time series data to illustrate changes over time.

In the `ggplot2` package, bar charts can be created using `geom_bar()` and `geom_col()`. The former automatically computes the count of observations for each category, requiring only the categorical variable as input. The latter is used when the data have already been summarised, in which case both the x- and y-variables must be explicitly specified. The general structure of these functions is shown below.

```{r geom-bar-code, echo=T, eval=F}
geom_bar(stat=, position=, color=, linetype=, linewidth=, fill=, alpha=, width=)
geom_col(position=, color=, linetype=, linewidth=, fill=, alpha=, width=)
```

The settings of all arguments in both `geom_bar()` and `geom_col()` function are shown in Table \@ref(tab:geom-bar-code-tab)。

```{r geom-bar-code-tab, echo=F, eval=T}
gb_code=data.frame(argument=c("`stat=`","`position=`","`color=`","`linetype=`","`linewidth=`","`fill=`","`alpha=`","`width=`"), setting=c("Set the statistical transformation", "Set whether subcategories are stacked or displayed side-by-side", "Set the outline colour of the bars", "Set the line type of the bar outlines", "Set the thickness of the bar outlines", "Set the fill colour of the bars", "Set the transparency of the bar fill", "Set the width of the bars"), explain=c("`count`: counts the number of observations in each category<br>`identity`: plots values as provided in the data<br>(only valid in `geom_bar()`)", "`stack`: stacks subcategories<br>`dodge`: places subcategories side-by-sides", "Refer to <A href='#geom-setting-color'>colour settings</A>", "Refer to <A href='#geom-setting-linetype'>line type settings</A>", "Numeric value", "Refer to <A href='#geom-setting-color'>colour settings</A>", "Numeric value between 0 (fully transparent) and 1 (fully opaque)", "Numeric value"))

kable(gb_code, booktabs=T, escape=F, caption="Argument settings in `geom_bar()` and `geom_col()`", col.names=c("Arguments","Usage","Note"))%>%
  kable_styling(bootstrap_options=c("striped", "hover"), font_size=14)%>%
  column_spec(1:3 ,extra_css="vertical-align:middle;")%>%
  row_spec(0, bold=T, color="white", background="#8E8E8E")
```

<p style="color:#003D79;font-size:18px;line-height:2">**⌾ Basic bar chart (automatic counting)**</p>
By default, the `geom_bar()` function uses `stat="count"`, meaning that only the variable to be counted (typically a categorical variable) needs to be specified. The function will automatically calculate the number of observations in each category and generate the corresponding bar chart.

Using the `mpg` dataset as an example, the x-axis is set to the categorical variable representing car manufacturers (`manufacturer`). Under the default setting, the function will automatically count the number of observations in each category. The code is written as follows.
...
<p style="color:#003D79;font-size:18px;line-height:2">**⌾ Basic bar chart (pre-aggregated data)**</p>
When summary statistics are already available, we only need to specify the categorical variable on the x-axis and the corresponding values (e.g., counts) on the y-axis. In this case, the `geom_bar()` function should be used with the argument `stat="identity"`, indicating that the values are provided directly and no additional counting (`count`) is required.

Using the `mpg` dataset as an example, the number of observations for each group (`manufacturer`) is first computed using the [`group_by() %>% summarise()`](#group-and-summarise) function. The resulting aggregated data are then visualised using `geom_bar()`. The code is written as follows.
...
To modify the order of the categorical variable on the x-axis, please refer to [this](#adjust-legend).

In addition, `geom_col()` provides a more straightforward approach for creating this type of bar chart, as it does not require additional arguments. Using the `mpg_sum` dataset from the previous example, the code is written as follows.

```{r geom-bar-eg3, echo=T, eval=T, out.width="70%", fig.align="center"}
# plot the bar chart using geom_col function
ggplot(data=mpg_sum, aes(x=manufacturer, y=Count))+
  geom_col()
```
...
<!-- 關於圖表中插入文字，應利用`geom_text()`函式，請先參考[此](#標註文字與標籤)。在繪製長條圖時，有時會希望在長條圖上標註類別或數字，以明確顯示充分的資訊，增加可讀性。以前文`mpg_sum`資料為例，將各廠牌（manufacturer）類別的統計數量（Count）標註於長條的頂端。程式碼中在設定文字的標註位置時，將縱軸設定為各數量再加一，以避免與頂端的邊界重疊而影響判讀。 -->
...
<!-- 關於圖表中插入標籤，應利用`geom_text()`函式，請先參考[此](#標註文字與標籤)。而若欲在圓餅圖各區塊上分別標註文字，請參考圖\@ref(fig:geom-bar-cumsum-fig)關於縱軸數值計算邏輯之示意圖。另外，請務必在計算累積比例與縱軸數值前，先將資料**依據圖例層級排列**，否則標註的文字或標籤將無法座落於正確的位置上。 -->
...
<!-- ### 棒棒糖圖 -->
<!-- 棒棒糖圖（Lollipop Plot）的呈現與長條圖非常類似，惟長條是利用柱狀表示統計量，而棒棒糖圖則是利用線段表示之，並利用點表示總量，故兩圖的功能甚為類似。此外，在繪製圖表時，縱使圓餅圖是最廣為應用的圖表設計，然而在實務上圓餅圖對於讀者而言無法清楚辨明大小關係（需判斷角度或面積大小，但對人類而言極難判別），致使視覺化效果不彰。因此在類別數量較多或各類別比例相近時，建議使用棒棒圖尚能提升視覺化程度。以圖\@ref(fig:geom-lollipop-eg0)的圓餅圖與棒棒糖圖為例，可發現類別 C 與 D、類別 E 與 F 間，難以判別兩者的差異，無法在第一時間判別大小關係，而棒棒糖圖則可以很清楚比較類別間的差異，更有利於讀者辨識。 -->

<!-- ```{r geom-lollipop-eg0, echo=F, eval=T, out.width="90%", fig.align="center", fig.cap="圓餅圖與棒棒糖圖比較"} -->
<!-- # temp=data.frame(Category=LETTERS[1:6], Percentage=c(5,9,15,18,25,28)) -->
<!-- # p1=ggplot()+ -->
<!-- #   geom_col(data=temp, aes(x="", y=Percentage, fill=Category))+ -->
<!-- #   coord_polar("y", direction=1)+ -->
<!-- #   scale_fill_brewer(palette="Set2")+ -->
<!-- #   theme_void() -->
<!-- #  -->
<!-- # p2=ggplot()+ -->
<!-- #   geom_segment(data=temp, aes(x=0, y=Category, xend=Percentage, yend=Category))+ -->
<!-- #   geom_point(data=temp, aes(x=Percentage, y=Category))+ -->
<!-- #   xlab("Percentage (%)")+ -->
...
<!-- #  -->
<!-- # png("./figure/compare_pie.png", width=640*2, height=490*2, res=200) -->
<!-- # print(p1) -->
<!-- # dev.off() -->
<!-- # png("./figure/compare_lollipop.png", width=780*2, height=490*2, res=200) -->
<!-- # print(p2) -->
<!-- # dev.off() -->
<!-- include_graphics("./figure/compare_pie_lollipop.png") -->
<!-- ``` -->


<!-- 在 R 語言中若欲繪製棒棒糖圖，可利用節線函式（`geom_segment()`）繪製，請參見表\@ref(tab:geom-linecurve-code-tab)關於`geom_segment()`函式之設定。該函式中必須設定`x=`與`y=`引數分別表示起始端點位置的橫坐標與縱坐標，而`xend=`與`yend=`引數則設置末端點的位置。最後可利用`geom_point()`函式新增總量的點，以精確表達總量。根據上述說明，棒棒圖的繪製架構如下所示。 -->

<!-- ```{r geom-lollipop-code, echo=T, eval=F} -->
<!-- # 橫向棒棒糖圖 -->
<!-- ggplot()+ -->
<!--   geom_segment(data=資料, aes(x=0, y=類別, xend=數值, yend=類別))+ -->
<!--   geom_point(data=資料, aes(x=數值, y=類別)) -->

<!-- # 縱向棒棒糖圖 -->
<!-- ggplot()+ -->
<!--   geom_segment(data=資料, aes(x=類別, y=0, xend=類別, yend=數值))+ -->
<!--   geom_point(data=資料, aes(x=類別, y=數值)) -->
<!-- ``` -->


<!-- <p style="color:#003D79;font-size:18px;line-height:2">**⌾ 呈現數值（類似長條圖）**</p>   -->
<!-- 棒棒糖圖的功能可與長條圖類似，用以呈現各類別的數值大小。以`mtcars`資料為例，繪製橫向的棒棒糖圖，故設定`x=`引數為 0，而`xend=`引數設定為數值大小（如以下範例中 mpg 變數），`y=`與`yend=`引數則設定為類別變數（如以下範例為 Car 變數）。以`mtcars`資料為例，程式碼撰寫如下。 -->

<!-- ```{r geom-lollipop-eg1, echo=T, eval=T, out.width="70%", fig.align="center"} -->
<!-- # 使用mtcars資料，並加上車種名稱 -->
<!-- mtcars_temp=mtcars -->
<!-- mtcars_temp$Car=row.names(mtcars_temp) -->

<!-- ggplot()+ -->
<!--   geom_segment(data=mtcars_temp, aes(x=0, y=Car, xend=mpg, yend=Car))+ -->
<!--   geom_point(data=mtcars_temp, aes(x=mpg, y=Car)) -->
<!-- ``` -->


<!-- <p style="color:#003D79;font-size:18px;line-height:2">**⌾ 圖表依數值大小排序**</p>   -->
<!-- 此外，有時會希望圖表的類別依據數值大小排列，以提升觀察辨識的程度，可利用[`factor()`](#因子factor)函式調整之，程式碼撰寫如下。 -->

<!-- ```{r geom-lollipop-eg2, echo=T, eval=T, out.width="70%", fig.align="center"} -->
<!-- mtcars_temp=arrange(mtcars_temp, mpg) -->
<!-- mtcars_temp$Car=factor(mtcars_temp$Car, levels=mtcars_temp$Car) -->
...
<!-- 關於`geom_text()`函式在圖表上加註文字，請參考[此](#標註文字與標籤)；透過`scale_x_discrete()`函式調整座標軸名稱，請參考[此](#調整座標軸名稱順序)。 -->


<!-- <p style="color:#003D79;font-size:18px;line-height:2">**⌾ 彙整資料後繪製桑基圖**</p>   -->
<!-- 有時候資料型態並不見得如同前面的範例，將所有節點逐欄羅列並給予統計量，此時我們必須預先處理資料尚能繪製桑基圖。以 2023 年臺灣直轄市間的<span id="scmig">社會遷入與遷出資料（scmig）</span>  為例，其資料記錄結構如下。 -->
...
<!-- ### 雷達圖 -->
<!-- :::install -->
<!-- **請安裝`ggradar`與`fmsb`套件！**   -->
<!-- **`devtools::install_github("ricardo-bion/ggradar")`**   -->
<!-- **`install.packages("fmsb")`** -->
<!-- ::: -->

<!-- 雷達圖（Radar Chart）可用於比較不同類別間的在各屬性上表現的狀況，並比較各類別間的相對優勢與劣勢。該圖通常是由多邊形或圓形所構成，其中心點代表最低數值，多邊形的頂點或圓形的邊界意謂各屬性的最大值。在 R 語言中可以透過`ggradar`與`fmsb`兩套件繪製雷達圖。 -->
...
<!-- 三元圖可以表達不同的三個類別之間的比例關係，以<span id="tw-vote">臺灣 2024 年總統選舉資料（tw_vote）</span>為例，有三個政黨推派候選人，故相當適合透過三元圖表達投票結果。以下程式碼中先行讀取全臺各鄉鎮的總統投票結果資料（tw_vote），其中記錄總票數（Votes_Total），以及各政黨的得票率，政黨含括 KMT（國民黨）、DPP（民進黨）、TPP（民眾黨）三者。 -->
...
<!--                    setting=c("箭頭的角度","箭頭的長度","箭頭位置","箭頭類型"), para=c("數值（預設為 30°）",'利用`unit(數值, "單位")`設定之',"`last`:加在線段或曲線的末端（預設）<br>`first`:加在線段或曲線的起始端<br>`both`:加在線段或曲線的兩端點","`open`:開放箭頭<br>`closed`:封閉箭頭")) -->
...
<!-- 最後繪製圖表如下，其中利用`geom_curve()`函式標記臺中市到達其他直轄市的社會遷出，並透過線條粗細表示移動人口的量。此外透[`geom_point()`](#散布圖)函式標記各直轄市位置，且利用[`geom_text()`](#標註文字與標籤)函式標註直轄市名稱。 -->
...
<!-- 由以上繪圖結果可知，臺中市大部分的人口係移動至新北市，其次為桃園市、臺北市。 -->



<!-- ## 圖表文字與插圖 -->
<!-- 在圖表中有時會插入文字或圖片，以標示更明確的訊息或輔助圖表判讀，並增添視覺化效果。在 R 語言中可以利用`ggplot2`套件及其延伸的套件客製化插入文字與圖表，所有本節會使用到的套件與函式彙整如表\@ref(tab:tab-textfig-plot)。 -->

<!-- ```{r tab-textfig-plot, echo=F, eval=T} -->
<!-- textfig=data.frame(pack=c(rep("`ggplot2`", 2), rep("`ggrepel`", 2), rep("`ggtext`", 2), rep("`ggimage`", 1)), -->
<!--                    func=c("[`geom_text()`](#標註文字與標籤)","[`geom_label()`](#標註文字與標籤)","[`geom_text_repel()`](#文字避免重疊)`","[`geom_label_repel()`](#文字避免重疊)","[`geom_richtext()`](#html-文字)","[`geom_textbox()`](#html-文字)","[`geom_image()`](#標註插圖)"), -->
...
<!-- 「標註文字」係指在圖片上加入純文字，而「標註標籤」則是指加入具有背景、邊框的文字，範例如圖\@ref(fig:geom-text-eg0)所示所示。在`ggplot2`套件中，標註文字可利用`geom_text()`函式；標註標籤可利用`geom_label()`函式，兩函式的撰寫架構如下。 -->

<!-- ```{r geom-text-eg0, echo=F, eval=T, out.width="70%", fig.height=1, fig.width=5, fig.align="center", fig.cap="標註文字與標籤圖示"} -->
<!-- temp=data.frame(x=c(0,1,0,1), y=c(0,0,1,1), -->
<!--                 nam=c("標註文字", "標註標籤", "`geom_text()`", "`geom_label()`")) -->
<!-- ggplot()+ -->
<!--   geom_label(aes(x=0, y=1, label="geom_text()"), fill="#D0D0D0", family="A", size=3, label.size=0)+ -->
<!--   geom_label(aes(x=1, y=1, label="geom_label()"), fill="#D0D0D0", family="A", size=3, label.size=0)+ -->
<!--   geom_text(aes(x=0, y=0, label="標註文字"), color="red", size=6, family="A")+ -->
<!--   geom_label(aes(x=1, y=0, label="標註標籤"), color="red", size=6, family="A", label.size=1, fill="#D2E9FF")+ -->
...
<!-- geom_label(aes(x=, y=, label=), color=, fill=, size=, alpha=, family=, fontface=, hjust=, vjust=, label.r=, label.size=) -->
<!-- ``` -->

<!-- `geom_text()`與`geom_label()`函式中的各引數設定請參考表\@ref(tab:tab-geom-text)。 -->

<!-- ```{r tab-geom-text, echo=F, eval=T} -->
<!-- gt_code=data.frame(argument=c("`x=`","`y=`","`label=`","`color=`","`fill=`","`size=`","`alpha=`","`angle=`","`family=`","`fontface=`","`hjust=`","`vjust=`","`nudge_x=`","`nudge_y=`","`label.r=`","`label.size=`"), -->
<!--                    setting=c('橫軸座標（必須<A href="#ggplot-aes">`aes()`</A>函式內）','縱軸座標（必須<A href="#ggplot-aes">`aes()`</A>函式內）','標註文字（必須<A href="#ggplot-aes">`aes()`</A>函式內）',"文字顏色（與標籤邊框的顏色）","標籤內部填色","文字大小","文字透明度","文字角度","文字字體","文字字型","文字橫軸對齊","文字縱軸對齊","文字橫軸位移","文字縱軸位移","標籤邊框的四角半徑","標籤邊框粗細"), explain=c("橫軸座標欄位","縱軸座標欄位","標註文字欄位","請參考<A href='#geom-setting-color'>顏色設定</A>","請參考<A href='#geom-setting-color'>顏色設定</A>","數值","數值，介於 0（完全透明）與 1（完全不透明）間","數值，介於 0° 至 360° 間","請參見[文字字體與字型](#文字字體與字型)","`plain`:一般（預設）<br>`bold`:粗體<br>`italic`:斜體<br>`bold.italic`:粗斜體","**數值：**`0`:靠左；`0.5`:置中；`1`:靠右<br>**文字：**`left`:靠左；`middle`:置中；`right`:靠右","**數值：**`0`:靠下；`0.5`:置中；`1`:靠上<br>**文字：**`bottom`:靠下；`center`:置中；`top`:靠上","將文字橫向移動單位數","將文字縱向移動單位數",'數值','以`units(數值, "單位")`設定之'), fun1=c(rep("✓", 4), "✗", rep("✓", 9), rep("✗", 2)), fun2=c(rep("✓", 7), "✗", rep("✓", 8))) -->

<!-- kable(gt_code, booktabs=T, escape=F, caption="`geom_text()`與`geom_label()`函式引數設定", col.names=c("引數","功能設定","說明","<span style='color:black'>`geom_text()`</span>","<span style='color:black'>`geom_label()`</span>"), align="lllcc")%>% -->
<!--   kable_styling(bootstrap_options=c("striped", "hover"), font_size=14)%>% -->
<!--   column_spec(1:5 ,extra_css="vertical-align:middle;")%>% -->
...
<!-- 以下範例展示設定字體與字型的方法，再次以 <A href='#tw-vote'>2024 年總統大選</A>資料為例，並萃取新竹縣的資料，繪製各鄉鎮的政黨得票率比例長條圖，最後在長條上標註得票比例之標籤（`geom_label()`）及行政區名稱之文字（`geom_text()`），程式碼撰寫如下。 -->

<!-- ```{r geom-text-eg1, echo=T, eval=F, warning=F} -->
<!-- # 先在系統中設定字體 -->
<!-- windowsFonts(biaukai=windowsFont("標楷體")) -->
<!-- windowsFonts(TNR=windowsFont("Times New Roman")) -->

<!-- # 讀取投總統大選資料並篩選出新竹市資料(計算邏輯請參考長條圖中「加入文字或標籤」的範例) -->
<!-- hs_vote=read.csv("https://raw.githubusercontent.com/ChiaJung-Yeh/Transport-Analysis/master/Data/tw_president_vote.csv")%>% -->
<!--   filter(COUNTYNAME=="新竹縣")%>% -->
<!--   melt(id.vars="TOWNNAME", measure.vars=c("TPP","DPP","KMT"), variable.name="Party", value.name="Rate") -->
<!-- hs_vote$Party=factor(hs_vote$Party, levels=c("TPP","DPP","KMT")) -->
<!-- hs_vote=group_by(hs_vote, TOWNNAME)%>% -->
<!--   arrange(TOWNNAME, desc(Party))%>% -->
<!--   mutate(Rate_cum=cumsum(Rate)) -->
<!-- hs_vote$lab_y=hs_vote$Rate_cum-hs_vote$Rate/2 -->

<!-- # 繪製圖表時設定family、fontface、hjust、nudge_y引數 -->
<!-- ggplot()+ -->
<!--   geom_col(data=hs_vote, aes(x=TOWNNAME, y=Rate, fill=Party), width=0.5)+ -->
<!--   geom_label(data=hs_vote, aes(x=TOWNNAME, y=lab_y, label=paste0(round(Rate*100, 1), "%")), size=3, family="TNR", nudge_y=-0.05, color="white", fill="#7B7B7B")+ -->
<!--   geom_text(data=hs_vote, aes(x=TOWNNAME, y=1, label=TOWNNAME), size=3, family="biaukai", angle=45, hjust=1) -->
<!-- ``` -->

<!-- ```{r geom-text-eg2, echo=F, eval=T, out.width="80%", fig.align="center", warning=F} -->
<!-- hs_vote=read.csv("https://raw.githubusercontent.com/ChiaJung-Yeh/Transport-Analysis/master/Data/tw_president_vote.csv")%>% -->
<!--   filter(COUNTYNAME=="新竹縣")%>% -->
<!--   melt(id.vars="TOWNNAME", measure.vars=c("TPP","DPP","KMT"), variable.name="Party", value.name="Rate") -->
<!-- hs_vote$Party=factor(hs_vote$Party, levels=c("TPP","DPP","KMT")) -->
...
<!-- ggplot()+ -->
<!--   geom_col(data=hs_vote, aes(x=TOWNNAME, y=Rate, fill=Party), width=0.5)+ -->
<!--   geom_label(data=hs_vote, aes(x=TOWNNAME, y=lab_y, label=paste0(round(Rate*100, 1), "%")), size=3, family="E", nudge_y=-0.05, color="white", fill="#7B7B7B")+ -->
<!--   geom_text(data=hs_vote, aes(x=TOWNNAME, y=1, label=TOWNNAME), size=3, family="D", angle=45, hjust=1) -->
<!-- ``` -->

<!-- 關於資料處理的過程請參考[長條圖](#長條圖)中「加入文字或標籤」的範例，而在繪圖函式中，利用以下引數設定字體與樣式： -->

<!-- * [`family=`](#文字字體與字型):設定文字的字體，百分比數值使用 Times New Roman；鄉鎮市區名稱則用標楷體   -->
<!-- * [`angle=`](#文字角度與對齊):設定文字角度，鄉鎮市區名稱以 45° 標記    -->
<!-- * [`hjust=`](#文字角度與對齊):設定文字對齊，鄉鎮市區名稱文字靠右對齊   -->
<!-- * [`nudge_y=`](#文字位移):設定文字位移，百分比數值標記統一往下 0.05 單位   -->

<!-- **各引數的設定請參見[3.2.2](#文字字體與字型)至[3.2.4](#文字位移)章節！** -->


<!-- ### 文字字體與字型 -->
<!-- :::note -->
<!-- 任何有關文字設定的套件通常皆可透過本小節所述方式設定**字體與字型**，包含以下函式： -->

<!-- * <u>**標註文字與標籤**</u>：[`geom_text()`](#標註文字與標籤)、[`geom_label()`](#標註文字與標籤)   -->
<!-- * <u>**標註 html 文字與標籤**</u>：[`geom_richtext()`](#html-文字)、[`geom_textbox()`](#html-文字)   -->
<!-- * <u>**設定圖表主題**</u>：[`element_text()`](#細節樣式設定)、[`element_markdown()`](#細節樣式設定)   -->
<!-- ::: -->

<!-- 在 Windows 系統中，若欲設定字體，可利用以下函式設定之： -->

<!-- ```{r font-family-code, echo=T, eval=T, out.width="70%", fig.align="center", warning=F} -->
<!-- windowsFonts(變數名稱=windowsFont("字體名稱")) -->
<!-- ``` -->

<!-- 設定完成後即可透過`family="變數名稱"`之引數設定圖片中的文字字體。而字型的設定可以透過`fontface=`設定之（惟主題樣式設定中的[`element_text()`](#細節樣式設定)是`face=`引數），包含原形（`plain`，預設）、粗體（`bold`）、斜體（`italic`）與粗斜體（`bold.italic`）。圖\@ref(fig:font-family-type)列舉數個字體與字型。 -->

<!-- ```{r font-family-type, echo=F, eval=T, out.width="75%", fig.align="center", fig.cap="調整文字字體與字型"} -->
<!-- temp=data.frame(family=c("Times New Roman","Consolas","標楷體","微軟正黑體"), -->
<!--                 fontface=c("plain","bold","italic","bold.italic"))%>% -->
<!--   expand.grid()%>% -->
<!--   left_join(data.frame(family_code=c("E","B","D","C"), -->
<!--                        label_eg=c("Times New Roman","Consolas","標楷體","微軟正黑體"), -->
<!--                        family=c("Times New Roman","Consolas","標楷體","微軟正黑體")), by=c("family")) -->
<!-- ggplot(data=temp, aes(x=family, y=fontface))+ -->
<!--   geom_text(aes(fontface=fontface, family=family_code, label=family), size=5)+ -->
<!--   xlab("字體") + ylab("字型")+ -->
<!--   theme(axis.text=element_text(family="A", size=12, face="bold", color="#003D79"), -->
<!--         axis.title=element_text(family="A", size=15, face="bold", color="#003D79"), -->
<!--         axis.ticks=element_blank()) -->
<!-- ``` -->

<!-- 本節係介紹 Windows 系統的字體設定方式，若在 Mac 系統中即無法透過`windowsFonts()`函式設定。R 語言中無論何系統，皆可透過`showtext`套件設定字體，該套件能夠更彈性地擴充字體，詳情請見[此](https://cran.rstudio.com/web/packages/showtext/vignettes/introduction.html)。 -->


<!-- ### 文字角度與對齊 -->
<!-- :::note -->
<!-- 任何有關文字設定的套件通常皆可透過本小節所述方式設定**角度與對齊**，包含以下函式： -->
...
<!-- 所有有關文字角度與對齊的函數，其引數設定方式皆同。調整角度利用`angle=`引數；調整對齊則利用`hjust=`與`vjust`引數，各引數設定參見表\@ref(tab:font-angle-just-tab)，設定結果參照圖\@ref(fig:font-angle-just)所示。 -->

<!-- ```{r font-angle-just-tab, echo=F, eval=T} -->
<!-- gt_code=data.frame(argument=c("`angle=`","`hjust=`","`vjust=`"), -->
<!--                    setting=c("文字角度","文字橫軸對齊","文字縱軸對齊"), explain=c("數值，介於 0° 至 360° 間","**數值：**`0`:靠左；`0.5`:置中；`1`:靠右<br>**文字：**`left`:靠左；`middle`:置中；`right`:靠右","**數值：**`0`:靠下；`0.5`:置中；`1`:靠上<br>**文字：**`bottom`:靠下；`center`:置中；`top`:靠上")) -->

<!-- kable(gt_code, booktabs=T, escape=F, caption="角度與對齊引數設定", col.names=c("引數","功能設定","說明"))%>% -->
<!--   kable_styling(bootstrap_options=c("striped", "hover"), font_size=14)%>% -->
<!--   column_spec(1:3 ,extra_css="vertical-align:middle;")%>% -->
<!--   row_spec(0, bold=T, color="white", background="#8E8E8E") -->
<!-- ``` -->

<!-- ```{r font-angle-just, echo=F, eval=T, out.width="100%", fig.width=15, fig.height=5, fig.align="center", fig.cap="調整文字角度與對齊"} -->
<!-- temp=expand.grid(angle=c(0,45,90), hjust=seq(0,1,0.5), vjust=seq(0,1,0.5)) -->

<!-- ggplot(data=temp)+ -->
<!--   geom_text(aes(x=hjust, y=vjust, angle=angle, hjust=hjust, vjust=vjust, label="NYCU"), fontface="bold", family="C", size=7)+ -->
<!--   geom_point(aes(x=hjust, y=vjust), size=3, color="red")+ -->
<!--   xlab("hjust") + ylab("vjust")+ -->
<!--   scale_x_continuous(expand=c(0.2,0.2))+ -->
<!--   scale_y_continuous(expand=c(0.2,0.2))+ -->
<!--   facet_grid(~angle)+ -->
<!--   theme(axis.text=element_text(family="B", size=20, face="bold"), -->
<!--         axis.title=element_text(family="B", size=20, face="bold"), -->
<!--         axis.ticks=element_blank(), -->
<!--         strip.text=element_text(family="B", size=20)) -->
<!-- ``` -->


<!-- ### 文字位移 -->
<!-- :::note -->
<!-- 任何有關文字設定的套件通常皆可透過本小節所述方式設定**文字位移**，包含以下函式： -->
...
<!-- 文字位移是指將所有文字或標籤往特定方向移動，如此可統一修正文字的位置。在繪圖的時候若希望永遠標註在各點的同一側，則適合利用`nudge_x`與`nudge_y`修正之。`nudge_x`是沿著橫軸往右（+）或往左（-）移動；`nudge_y`是沿著縱軸往上（+）或往下（-）移動，詳請參見圖\@ref(fig:font-nudge)。 -->

<!-- ```{r font-nudge, echo=F, eval=T, out.width="100%", fig.width=10, fig.height=3, fig.align="center", fig.cap="文字位移", warning=F} -->
<!-- temp1=data.frame(x=c(0,2,1), y=c(0,0,2)) -->
<!-- temp2=data.frame(nx=c(-0.5,0,0.5), ny=c(-0.5,0,0.5)) -->
<!-- p_list=list() -->
<!-- for(i in c(1:nrow(temp2))){ -->
<!--   p=ggplot()+ -->
<!--     geom_point(data=temp1, aes(x=x, y=y))+ -->
<!--     geom_text(data=temp1, aes(x=x, y=y, label="NYCU"), nudge_x=temp2$nx[i], nudge_y=temp2$ny[i])+ -->
<!--     scale_x_continuous(limits=c(-1,3), breaks=seq(-1,3,0.5))+ -->
<!--     scale_y_continuous(limits=c(-1,3), breaks=seq(-1,3,0.5))+ -->
<!--     ggtitle(label=paste0('<span style="font-family:B">nudge_x=', temp2$nx[i], "<br>nudge_y=", temp2$ny[i], "</span>"))+ -->
<!--     theme_minimal()+ -->
...
<!-- 在`ggplot2`套件中，若有許多文字標註，很容易產生重疊的情形，此時可以安裝`ggrepel`套件，利用該套件中的`geom_text_repel()`與`geom_label_repel()`函式修正之，兩函式的設定引數與`geom_text()`及`geom_label()`函式大部分皆相同，惟能自動化調整文字或標籤的位置以避免重疊。關於此二函式額外的重要引數設定彙整如表\@ref(tab:ggrepel-code-tab)。 -->

<!-- ```{r ggrepel-code-tab, echo=F, eval=T, out.width="70%", fig.align="center"} -->
<!-- temp=data.frame(para=c("`min.segment.length=`","`direction=`","`arrow=`","`seed=`"), -->
<!--                 setting=c("低於此一距離時，即繪製指示線","自動修正的方向","設定指示線的箭頭圖樣","設定隨機種子"), -->
<!--                 explain=c('利用`unit(數值, "單位")`設定之<br>若欲強制新增指示線，可設定`unit(0, "cm")`',"`both`:同時修正橫軸與縱軸<br>`x`:修正橫軸<br>`y`:修正縱軸","利用`arrow()`函式設定之（參見表\\@ref(tab:geom-arrow-code-tab)）","由於修正的位移為隨機，故若希望每次繪圖結果皆固定，可設定隨機種子編號")) -->

<!-- kable(temp, booktabs=T, escape=F, caption="`geom_text_repel()`與`geom_label_repel()`函式其他引數設定", col.names=c("引數","功能設定","說明"))%>% -->
<!--   kable_styling(bootstrap_options=c("striped", "hover"), font_size=14)%>% -->
<!--   column_spec(1:3 ,extra_css="vertical-align:middle;")%>% -->
<!--   row_spec(0, bold=T, color="white", background="#8E8E8E") -->
<!-- ``` -->

<!-- <p style="color:#003D79;font-size:18px;line-height:2">**⌾ 修正重疊文字**</p>   -->

<!-- 以`swiss`資料為例，該資料記錄西元 1888 年 47 個省份的生育率（Fertility）與社經變數，包括農業人口比例（Agriculture）、小學畢業人口比例（Education）、基督教人口比例（Catholic）等。以下程式碼繪製生育率與農業人口比例之間的關係，並單純使用`geom_text()`函式標記省份名稱。 -->

<!-- ```{r ggrepel-eg1, echo=T, eval=T, out.width="80%", fig.align="center"} -->
<!-- swiss_temp=swiss -->
<!-- swiss_temp$Province=row.names(swiss_temp) -->
<!-- ggplot()+ -->
<!--   geom_point(data=swiss_temp, aes(x=Fertility, y=Agriculture))+ -->
<!--   geom_text(data=swiss_temp, aes(x=Fertility, y=Agriculture, label=Province)) -->
<!-- ``` -->

<!-- 由出圖結果可知，標記的名稱有許多重疊之處，造成判讀不易，故以下進一步透過`geom_text_repel()`函式修正之，程式碼撰寫如下。 -->

<!-- ```{r ggrepel-eg2, echo=T, eval=T, out.width="80%", fig.align="center"} -->
<!-- ggplot()+ -->
<!--   geom_point(data=swiss_temp, aes(x=Fertility, y=Agriculture))+ -->
<!--   geom_text_repel(data=swiss_temp, aes(x=Fertility, y=Agriculture, label=Province)) -->
<!-- ``` -->

<!-- 利用`geom_label_repel()`函式標註不重疊的標籤，程式碼撰寫如下。 -->

<!-- ```{r ggrepel-eg3, echo=T, eval=T, out.width="80%", fig.align="center"} -->
<!-- ggplot()+ -->
<!--   geom_point(data=swiss_temp, aes(x=Fertility, y=Agriculture))+ -->
<!--   geom_label_repel(data=swiss_temp, aes(x=Fertility, y=Agriculture, label=Province)) -->
<!-- ``` -->


<!-- <p style="color:#003D79;font-size:18px;line-height:2">**⌾ 新增指示線與箭頭**</p>   -->
<!-- 透過`min.segment.length=unit(0, "cm")`強制新增指示線，並透過`arrow()`函式設定箭頭樣式，程式碼撰寫如下。 -->

<!-- ```{r ggrepel-eg4, echo=T, eval=T, out.width="80%", fig.align="center"} -->
<!-- ggplot()+ -->
<!--   geom_point(data=swiss_temp, aes(x=Fertility, y=Agriculture))+ -->
<!--   geom_text_repel(data=swiss_temp, aes(x=Fertility, y=Agriculture, label=Province), -->
<!--                   min.segment.length=unit(0, "cm"), arrow=arrow(length=unit(0.1, "cm"), type="closed")) -->
<!-- ``` -->
...
<!-- `geom_text()`與`geom_label()`函式皆存在其限制，無法在同一函式中設定多個不同的樣式。舉例而言，若設定文字字體為標楷體，則無論中文或英文皆會以標楷體呈現，然而有時候會希望英文使用 Times New Roman 字體，此時即無法單純藉`geom_text()`與`geom_label()`函式設定之。`ggtext`套件中的`geom_richtext()`與`geom_textbox()`兩函式可更彈性地提供透過 HTML 語法設定文字樣式，此外亦可利用`<span style=''></span>`之 HTML 標籤增加文字樣式的豐富度。常用的 HTML 文字樣式設定彙整如表\@ref(tab:tab-html-font)，部分 HTML 標籤在本套件仍無法使用（如`<u></u>`表示底線，但未支援之）。 -->

<!-- ```{r tab-html-font, echo=F, eval=T} -->
<!-- html_span=read.csv("./Data/html_tag.csv") -->
<!-- kable(html_span, booktabs=T, escape=F, caption="HTML 文字標籤設定", col.names=c("調整樣式","HTML 語法"), align=c("cl"))%>% -->
<!--   kable_styling(bootstrap_options=c("striped", "hover"), font_size=14)%>% -->
<!--   row_spec(0, bold=T, color="white", background="#8E8E8E") -->
<!-- ``` -->

<!-- HTML 標籤中`style`參數可直接以分號（`;`）分隔多重設定樣式，例如同時設定文字大小和顏色，則標籤寫法如下： -->
<!-- ```{r tab-html-code-eg, echo=T, eval=F} -->
<!-- <span style='font-size:18mm;color:red'>欲呈現的文字</span> -->
<!-- ``` -->

<!-- `geom_richtext()`與`geom_textbox()`兩函式內所有參數分別與`geom_text()`與`geom_label()`幾乎一致，請參考表\@ref(tab:tab-geom-text)。在函式中的設定為基底樣式，而若在 HTML 標籤中有另外設定字體樣式，則會優先採用 HTML 標籤之設定。`geom_richtext()`與`geom_textbox()`兩者的設定結果幾乎一致，惟部分參數之特殊設定彙整如表\@ref(tab:geom-rich-setting)。 -->

<!-- ```{r geom-rich-setting, echo=F, eval=T} -->
<!-- geom_set=data.frame(setting=c("`color=`","`width=`","`box.color=`","`box.size=`"), description=c("設定文字和外圍方框的顏色", "標籤的總寬度"," 外圍方框的顏色", "外圍方框的粗細"), function1=c("V","X","X","X"), function2=c("V","V","V","V")) -->

<!-- kable(geom_set, booktabs=T, escape=F, caption="HTML 文字標籤設定", col.names=c("引數","設定說明","geom_richtext()","geom_textbox()"), align=c("llcc"))%>% -->
<!--   kable_styling(bootstrap_options=c("striped", "hover"), font_size=14)%>% -->
<!--   row_spec(0, bold=T, color="white", background="#8E8E8E") -->
<!-- ``` -->

<!-- 以下範例分別利用`geom_richtext()`與`geom_textbox()`函式建立圖表上的文字標籤。在`geom_richtext()`函式中，基底樣式為紅色字體與框線（`color="red"`），以淺黃色填色（`fill="#FFFFAA"`），並設定字體為標楷體。而`text1`內文中額外設定該字串的字體為「Consolas」，顏色為「#AE00AE」。只有在`text1`字串中的文字，會依據其 HTML 的標籤設定，除此之外將會使用函式中的基底樣式。`text2`的字串中則利用`<b></b>`標籤將自型設定為粗體，並調整字體大小為「20pt」。只有在`text2`內的字串會使用粗體字，其餘則亦使用基底樣式。在`geom_textbox()`函式中的設定與上述雷同，惟另添加引數`width=`用以設定整個標籤的長度，若文字並未填滿整個方框則會予以空白；新增`box.color=`引數設定外框的顏色；`box.size=`引數用以調整框線的粗度。程式碼中的`hjust=0`表示靠左對齊；`hjust= -->
<!-- 1`表示靠右對齊，請參見表\@ref(tab:font-angle-just-tab)。 -->

<!-- ```{r tab-html-eg1, echo=T, eval=T} -->
<!-- windowsFonts(Consolas=windowsFont("Consolas")) -->
<!-- windowsFonts(biaukai=windowsFont("標楷體")) -->

<!-- # 設定文字顏色與字體 -->
<!-- text1="<span style='color:#AE00AE;font-family:Consolas'>geom_richtext()</span>" -->

<!-- # 設定粗體與文字大小 -->
<!-- text2="<b><span style='font-size:20pt'>函式</span></b>" -->

<!-- # 設定文字顏色與字體 -->
<!-- text3="<span style='color:#2894FF;font-family:Consolas'>geom_textbox()</span>" -->

<!-- ggplot()+ -->
<!--   geom_richtext(aes(x=-1, y=0, label=paste0(text1, text2)), color="red", fill="#FFFFAA", hjust=0, family="biaukai")+ -->
<!--   geom_textbox(aes(x=1, y=0, label=paste0(text3, text2)), color="red", hjust=1, width=0.4, box.color="green", box.size=1, family="biaukai") -->
<!-- ``` -->
...
<!-- 在繪圖時可能會希望以圖示取代常見的圖形（如預設的圓點或 R 語言<A href='#geom-setting-shape'>基本圖示</A>），則可利用標註插圖的方式，將圖檔疊加在`ggplot`所生成的圖上。在 R 語言中可利用`ggimage`套件附加圖片，該函式撰寫架構如下。函式中`image=`引數可以使用本機的圖片連結，亦可使用網路圖片連結。   -->

<!-- ```{r geom-image-code, echo=T, eval=F} -->
<!-- geom_image(image=, size=) -->
<!-- ``` -->

<!-- 以網路連結圖片為範例，以下程式碼先建立`nycu_logo`之資料，其中儲存圖片所在連結。 -->

<!-- ```{r geom-image-eg, echo=T, eval=T} -->
<!-- # 建立圖片連結之data frame -->
<!-- nycu_logo=data.frame(x=c("English","Mandarin"), y=c(0,0), -->
<!--                      link=c("https://www.nycu.edu.tw/userfiles/nycuch/images/20230915175559100.png", -->
<!--                             "https://www.nycu.edu.tw/userfiles/nycuch/images/20230915175431239.png")) -->

<!-- # 利用geom_image繪圖 -->
<!-- ggplot(data=nycu_logo, aes(x=x, y=y))+ -->
<!--   geom_image(aes(image=link), size=0.8) -->
<!-- ``` -->


<!-- ## 繪圖設定 -->
<!-- 在前述介紹各類型圖表繪製時，著重於說明繪製的方式，並未著墨圖表的細節。然若希望提升視覺化的呈現，應進一步調整圖表的細節。本節將依序介紹[圖例與座標軸的設定](#圖例與座標軸設定)、[圖中點與線的樣式調整](#尺度設定)、整張圖片的樣式調整(#主題設定)。 -->

<!-- ```{r geom-setting-eg-plot, echo=F, eval=F} -->
<!-- ggplot()+ -->
<!--   geom_point(data=iris, aes(x=Sepal.Length, y=Petal.Width, color=Species), size=2) -->

<!-- ggplot()+ -->
<!--   geom_point(data=iris, aes(x=Sepal.Length, y=Petal.Width, color=Species), size=2)+ -->
<!--   scale_color_brewer(palette="Set2", name="品種")+ -->
<!--   scale_x_continuous(limits=c(4,8))+ -->
<!--   scale_y_continuous(limits=c(0,3))+ -->
<!--   xlab("花萼長度") + ylab("花瓣寬度")+ -->
<!--   theme_bw()+ -->
<!--   theme(panel.background=element_rect(fill=alpha("#C4E1FF", 0.2)), -->
<!--         panel.border=element_rect(color="#005AB5", linewidth=1), -->
<!--         panel.grid.major=element_line(color="#9D9D9D", linewidth=0.1), -->
<!--         panel.grid.minor=element_line(color="#BEBEBE", linewidth=0.1, linetype="dashed"), -->
<!--         legend.title=element_text(family="A", size=14), -->
<!--         legend.text=element_text(family="A", size=12), -->
<!--         legend.background=element_rect(color="#A6A6D2", fill=alpha("#D8D8EB", 0.5)), -->
<!--         legend.key=element_rect(fill=alpha("#FFFFDF", 0.8)), -->
<!--         legend.spacing=unit(2, "cm"), -->
<!--         legend.position=c(0.88,0.2), -->
<!--         axis.title=element_text(family="A", size=15, face="bold"), -->
<!--         axis.text=element_text(family="A", size=12, color="#004B97"), -->
<!--         axis.ticks=element_line(color="#004B97", linewidth=1)) -->
<!-- ``` -->
